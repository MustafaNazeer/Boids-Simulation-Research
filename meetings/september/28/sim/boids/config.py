...
BOUNDS_MODES = ("reflect", "wrap", "none")
...
DEFAULTS = {
    ...
    "rules": {
        ...
        "food_seek": {"enabled": True, "radius": 20.0, "weight": 1.0},
    },
    "leaders": {"count": 3},
    "preplanned": {"count": 0, "speed": 5.0},
    "food": {"count": 0, "amount": 60, "eat_radius": 1.5, "respawn": True},
        ...
        for field in RULE_KEYS[name]:
            if rule[field] < 0:
                raise ValueError("rules.%s.%s must not be negative"
                                 % (name, field))
    if cfg["food"]["count"] < 0:
        raise ValueError("food.count must not be negative")
    if cfg["food"]["count"] > 0:
        if cfg["food"]["amount"] <= 0:
            raise ValueError("food.amount must be positive")
        if cfg["food"]["eat_radius"] <= 0:
            raise ValueError("food.eat_radius must be positive")
    return cfg
...
def to_params(cfg):
    ...
    params = {
        ...
        "food_sensing_radius": rules["food_seek"]["radius"],
        "food_eat_radius": cfg["food"]["eat_radius"],
        "food_amount": cfg["food"]["amount"],
        "food_respawn": bool(cfg["food"]["respawn"]),
