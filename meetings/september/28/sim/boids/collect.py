...
from sim.boids.leader import follow as leader_follow, NO_LEADER
...
MANIFEST_HEADER = [
    ...
    "n_food", "food_sensing_radius", "w_food_seek", "bounds",
]
...
def sample_params(rng):
    ...
    return {
        ...
        "n_food": int(rng.integers(flo, fhi + 1)),
        "food_sensing_radius": _uniform(rng, "food_sensing_radius"),
        "w_food_seek": _uniform(rng, "w_food_seek"),
    }


def build_world(cfg, run_seed, frames, world_size, base=None, bounds_mode="reflect"):
    ...
    params["food_sensing_radius"] = cfg["food_sensing_radius"]
    params["weights"]["food_seek"] = cfg["w_food_seek"]
    ...
    return World(positions, velocities, params, world_size,
                 ...
                 bounds_mode=bounds_mode, food_positions=food_positions,
                 food_amounts=food_amounts)


def manifest_row(cfg, run_id, run_seed, frames, dt, world_size, bounds):
    return {
        "run_id": run_id,
        "seed": run_seed,
        "n_boids": cfg["n_boids"],
        ...
        "n_food": cfg["n_food"],
        "food_sensing_radius": cfg["food_sensing_radius"],
        "w_food_seek": cfg["w_food_seek"],
        "bounds": bounds,
    }


def run_one(cfg, run_id, run_seed, frames, dt, world_size, out_dir, base=None,
           bounds="reflect", write_csv=True):
    world = build_world(cfg, run_seed, frames, world_size, base,
                        bounds_mode=bounds)
    # to_csv is the recorder's only consumer, so when no csv is wanted the
    # recorder is not built at all. Measured 2026-08-20 at 300 frames: 16
    # percent faster and peak traced memory down from 52.2 MB to 4.2 MB
    recorder = TrajectoryRecorder(dt) if write_csv else None
    positions_seq = []
    velocities_seq = []
    leader_ids_seq = []
    food_positions_seq = []
    ...
    for _ in range(frames):
        ...
        if has_leaders:
            ...
            lids = np.full(cfg["n_boids"], NO_LEADER, dtype=int)
        leader_ids_seq.append(lids)
        if world.food_positions is not None:
            food_positions_seq.append(world.food_positions.copy())
            food_amounts_seq.append(world.food_amounts.copy())
            _, fids = food_mod.seek(world.positions, world.food_positions,
                                    world.food_amounts,
                                    cfg["food_sensing_radius"])
        else:
            fids = np.full(cfg["n_boids"], food_mod.NO_FOOD, dtype=int)
        food_ids_seq.append(fids)
        world.step(dt)
    ...
    np.savez_compressed(
        ...
        seed=run_seed,
        frames=frames,
        food_ids=np.stack(food_ids_seq),
        food_positions=(np.stack(food_positions_seq)
                        if food_positions_seq else
                        np.zeros((frames, 0, 2))),
        food_amounts_seq=(np.stack(food_amounts_seq)
                          if food_amounts_seq else np.zeros((frames, 0))),
        n_food=cfg["n_food"],
        food_sensing_radius=cfg["food_sensing_radius"],
        w_food_seek=cfg["w_food_seek"],
        bounds=bounds,
    )
    return manifest_row(cfg, run_id, run_seed, frames, dt, world_size, bounds)
...
def main():
    ...
    rows = collect(args.runs, frames, dt, args.seed,
                   out_dir, world_size, base, bounds=cfg["world"]["bounds"])
