...
def follower_attachment(leader_ids, leader_mask):
    mask = np.asarray(leader_mask, dtype=bool)
    ids = np.asarray(leader_ids)
    followers = ~mask
    total = int(followers.sum())
    if total == 0:
        return 0.0
    return float((ids[followers] >= 0).sum()) / total
...
def food_attachment(food_ids):
    ids = np.asarray(food_ids)
    if ids.size == 0:
        return 0.0
    return float((ids >= 0).sum()) / ids.size
