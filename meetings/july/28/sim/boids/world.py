import numpy as np
from sim.boids import rules, predator as predator_mod, leader as leader_mod, food as food_mod
...
class World:
    ...
    def __init__(self, positions, velocities, params, world_size,
                 ...
                 leader_mask=None, food_positions=None, food_amounts=None, seed=None):
        ...
        self.food_positions = (None if food_positions is None else
                               np.asarray(food_positions, dtype=float).copy())
        self.food_amounts = (None if food_amounts is None else
                             np.asarray(food_amounts, dtype=float).copy())
        self.food_ids = np.full(n, food_mod.NO_FOOD, dtype=int)
    ...
    def _boid_acceleration(self):
        ...
        if self.food_positions is not None and self._enabled("food_seek"):
            food_accel, self.food_ids = food_mod.seek(
                p, self.food_positions, self.food_amounts,
                prm["food_sensing_radius"])
            accels.append(food_accel)
            weights.append(w["food_seek"])
        if not accels:
            return np.zeros_like(self.velocities)
        return rules.combine(accels, weights)

    def _respawn_location(self):
        # in the unbounded mode a uniform draw over the spawn box would strand
        # food behind a drifting flock, so respawn near the current centroid
        if self.bounds_mode == "none":
    ...
    def _eat_and_respawn(self):
        ...
        eating = ids >= 0
        if eating.any():
            rows = np.where(eating)[0]
            offsets = self.positions[rows] - self.food_positions[ids[rows]]
            dist = np.linalg.norm(offsets, axis=1)
            close = rows[dist <= self.params["food_eat_radius"]]
            if close.size:
                drained = np.bincount(
                    ids[close], minlength=self.food_amounts.size)
                self.food_amounts = np.maximum(
                    self.food_amounts - drained, 0.0)
        if self.params.get("food_respawn", True):
            for k in np.where(self.food_amounts <= 0)[0]:
                self.food_positions[k] = self._respawn_location()
                self.food_amounts[k] = self.params["food_amount"]
