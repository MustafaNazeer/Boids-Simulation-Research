...
def build_world(seed=7, n_boids=120, world_size=60.0, with_predator=True):
    ...
    return World(positions, velocities, default_params(), world_size, obstacles=obstacles, predator_state=predator_state)
...
def build_flock_with_leaders(seed, n_boids, world_size, n_leaders, frames):
    ...
    if n_leaders > 0:
        ...
        agents = preplanned_mod.PreplannedAgents(plans, frames, amplitude, center, params['preplanned_speed'])
        pp_pos, pp_vel = agents.state_at(0)
        positions[leader_ids] = pp_pos
        velocities[leader_ids] = pp_vel
    return World(positions, velocities, params, world_size, preplanned=agents, preplanned_mask=mask)
...
def build_preplanned_debug_world(world_size, frames):
    ...
    center = np.array([world_size / 2.0, world_size / 2.0])
    amplitude = world_size / 2.0 * 0.8
    agents = preplanned_mod.PreplannedAgents(preplanned_mod.PLAN_CATALOG, frames, amplitude, center, params['preplanned_speed'])
    pos0, vel0 = agents.state_at(0)
    mask = np.ones(agents.count, dtype=bool)
    return World(pos0.copy(), vel0.copy(), params, world_size, preplanned=agents, preplanned_mask=mask)
...
def main():
    ...
    world = build_world()
    render.save_animation(world, frames=300, dt=0.2, out_path=os.path.join(OUT, 'flock_escape.gif'))
    world = build_world()
    pol, nn, clusters = record_metrics(world, frames=300, dt=0.2)
    plot_series(pol, 'polarization', 'Flock polarization over time', os.path.join(OUT, 'polarization.png'))
    plot_series(nn, 'mean NN distance', 'Spacing over time (spikes on escape)', os.path.join(OUT, 'nn_distance.png'))
    plot_series(clusters, 'cluster count', 'Cluster count (rises on split)', os.path.join(OUT, 'cluster_count.png'))
    world = build_world()
    for _ in range(40):
        world.step(0.2)
    ...
    flock = build_flock_with_leaders(seed=7, n_boids=120, world_size=60.0, n_leaders=2, frames=300)
    record_trajectory(flock, frames=300, dt=0.2, out_path=os.path.join(OUT, 'trajectory.csv'))
    debug = build_preplanned_debug_world(world_size=60.0, frames=240)
    record_trajectory(debug, frames=240, dt=0.2, out_path=os.path.join(OUT, 'preplanned_debug.csv'))
    debug_anim = build_preplanned_debug_world(world_size=60.0, frames=240)
    render.save_animation(debug_anim, frames=240, dt=0.2, out_path=os.path.join(OUT, 'preplanned_debug.gif'))
    debug_for_plot = build_preplanned_debug_world(world_size=60.0, frames=240)
    save_preplanned_path_plot(debug_for_plot.preplanned, 60.0, os.path.join(OUT, 'snapshot_preplanned.png'))
