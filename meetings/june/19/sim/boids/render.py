...
def _draw_frame(ax, world):
    ...
    if mask is not None:
        ...
        normal = ~mask
        ax.scatter(world.positions[normal, 0], world.positions[normal, 1], s=7, c='steelblue', alpha=0.85, zorder=2)
        ax.scatter(world.positions[mask, 0], world.positions[mask, 1], s=22, c='gold', alpha=0.95, zorder=3)
    else:
        ax.scatter(world.positions[:, 0], world.positions[:, 1], s=7, c='steelblue', alpha=0.85, zorder=2)
    if world.predator is not None:
        ax.scatter([world.predator['pos'][0]], [world.predator['pos'][1]], s=60, c='crimson', marker='*', zorder=3)
